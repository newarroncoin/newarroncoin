// Copyright (c) 2014 The ArronCoin developers
...
ArronGUI::ArronGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new ArronBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("Arroncoin") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/arron"));
    setWindowIcon(QIcon(":icons/arron"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/arron/index.html");
#else
    QFile html("/opt/arron/index.html");
#endif
...
void ArronGUI::pageLoaded(bool ok)
{
...
void ArronGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);

}

void ArronGUI::urlClicked(const QUrl & link)
{
...
void ArronGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/arron"), tr("&About ArronCoin"), this);
    aboutAction->setToolTip(tr("Show information about ArronCoin"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for ArronCoin"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/arron"), tr("&Show / Hide"), this);
...
void ArronGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("Arroncoin") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/arron_testnet"));
            setWindowIcon(QIcon(":icons/arron_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/arron_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Arroncoin client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/arron_testnet"));
...
void ArronGUI::setWalletModel(WalletModel *walletModel)
{
...
void ArronGUI::setMessageModel(MessageModel *messageModel)
{
...
void ArronGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Arroncoin client"));
    trayIcon->setIcon(QIcon(":/icons/arron"));
...
#ifndef Q_OS_MAC
void ArronGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void ArronGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to ArronCoin network", "", count));
}

void ArronGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void ArronGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void ArronGUI::changeEvent(QEvent *e)
{
...
void ArronGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void ArronGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::ARC, nFeeRequired));
...
void ArronGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void ArronGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void ArronGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void ArronGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ArronGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void ArronGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid ArronCoin address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void ArronGUI::handleURI(QString strURI)
{
...
void ArronGUI::setEncryptionStatus(int status)
{
...
void ArronGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void ArronGUI::changePassphrase()
{
...
void ArronGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void ArronGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ArronGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void ArronGUI::updateStakingIcon()
{
