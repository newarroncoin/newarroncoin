...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return ArronGUI::tr("Received with");
    case RecvFromOther:
        return ArronGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return ArronGUI::tr("Sent to");
    case SendToSelf:
        return ArronGUI::tr("Payment to yourself");
    case Generated:
        return ArronGUI::tr("Mined");
    case RecvArron:
        return ArronGUI::tr("Received arron");
    case SendArron:
        return ArronGUI::tr("Sent arron");
    case Other:
        return ArronGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvArron:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendArron:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredARC, nCredArron;
    wtx.GetCredit(nCredARC, nCredArron, true);
    int64_t nCredit = nCredARC + nCredArron;
    ...
    {
        if (nNet > 0 && nCredArron > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvArron, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendArron, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvArron;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv arron
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendArron;
                    sub.address = CBitcoinAddress(ckidD).ToString();
