...
#include "arronbridge.moc"

ArronBridge::ArronBridge(ArronGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void ArronBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void ArronBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void ArronBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void ArronBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void ArronBridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void ArronBridge::populateOptions()
{
...
// Transactions
bool ArronBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void ArronBridge::clearRecipients()
{
    recipients.clear();
}

bool ArronBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
    ...
    {
        int inputType; // 0 ARC, 1 Arron
        switch(rcp.txnTypeInd)
        {
            case TXT_ARC_TO_ARC:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ARC, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_ARC_TO_ANON:
                formatted.append(tr("<b>%1</b> to ARRON %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ARC, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> ARRON, ring size %2 to ARRON %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ARC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ARC:
                formatted.append(tr("<b>%1</b> ARRON, ring size %2 to ARC %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ARC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ARC, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsArronBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your arron balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ARC, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void ArronBridge::openCoinControl()
{
...
void ArronBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void ArronBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap ArronBridge::listAnonOutputs()
{
...
void ArronBridge::populateTransactionTable()
{
...
void ArronBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void ArronBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString ArronBridge::transactionDetails(QString txid)
{
...
// Addresses
void ArronBridge::populateAddressTable()
{
...
void ArronBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void ArronBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString ArronBridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString ArronBridge::lastAddressError()
{
    QString sError;
...
QString ArronBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void ArronBridge::updateAddressLabel(QString address, QString label)
{
...
bool ArronBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool ArronBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void ArronBridge::appendMessages(QVariantList messages, bool reset)
{
    emitMessages(messages, reset);
}

void ArronBridge::appendMessage(int row)
{
...
void ArronBridge::populateMessageTable()
{
...
void ArronBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool ArronBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool ArronBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString ArronBridge::getPubKey(QString address)
{
    return addressModel->atm->pubkeyForAddress(address);
}

QString ArronBridge::addressForPubKey(QString pubkey)
{
    return addressModel->atm->addressForPubkey(pubkey);
}

bool ArronBridge::setPubKey(QString address, QString pubkey)
{
...
bool ArronBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
...
QString ArronBridge::createGroupChat(QString label)
{
...
QString ArronBridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList ArronBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
void ArronBridge::connectSignals()
{
...
QString ArronBridge::translateHtmlString(QString string)
{
    int i = 0;
...
QVariantMap ArronBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap ArronBridge::listLatestBlocks()
{
...
QVariantMap ArronBridge::findBlock(QString searchID)
{
...
QVariantMap ArronBridge::blockDetails(QString blkHash)
{
...
QVariantMap ArronBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap ArronBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap ArronBridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap ArronBridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap ArronBridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap ArronBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap ArronBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap ArronBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap ArronBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with Arroncoin prefix.");
            return result;
        }
...
QVariantMap ArronBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap ArronBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap ArronBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
