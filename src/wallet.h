...
public:
    ...
    int64_t GetArronDebit(const CTxIn& txin) const;
    int64_t GetArronCredit(const CTxOut& txout) const;
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nARC, int64_t& nArron) const
    {
        nARC = 0;
        nArron = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nArron += GetArronCredit(txout);
            } else
                nARC += GetCredit(txout);
            if (!MoneyRange(nARC)
                || !MoneyRange(nArron))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
...
public:
    ...
    mutable bool fAvailableArronCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableArronCreditCached;
    
    mutable int64_t nCredARCCached;
    mutable int64_t nCredArronCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableArronCreditCached = false;
    ...
    bool GetCredit(int64_t& nCredARC, int64_t& nCredArron, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredARC = nCredArron = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredARCCached = 0;
            nCredArronCached = 0;
            pwallet->GetCredit(*this, nCredARCCached, nCredArronCached);
            fCreditSplitCached = true;
        };
        
        nCredARC = nCredARCCached;
        nCredArron = nCredArronCached;
        return true;
    }
    ...
    int64_t GetAvailableArronCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableArronCreditCached)
            return nAvailableArronCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableArronCredit() : value out of range");
            };
        };
        
        nAvailableArronCreditCached = nCredit;
        fAvailableArronCreditCached = true;
        return nCredit;
    };
